...
async function handleMessage(message: Message) {
  ...
  if (stateResult.value) {
    ...
    if (state.name === 'broadcast_awaiting_content' && user.id.toString() === ADMIN_CHAT_ID) {
    ...
    if (state.name === 'awaiting_lesson_details') {
      ...
      await sendMessage(user.id, `? ??? *${parts[0]}* ?? ?????? ????? ??!`);
      return;
    }
  }
...
async function handleCallbackQuery(query: CallbackQuery) {
  ...
  // =================================================================
  //  START OF THE FIX: ADDED ADMIN SECURITY CHECK
  // =================================================================
  const isAdminCallback = main === 'admin' || main === 'broadcast';
  if (isAdminCallback && user.id.toString() !== ADMIN_CHAT_ID) {
      log("WARN", `Unauthorized attempt to access admin callback by user ${user.id}`, { data });
      await answerCallbackQuery(query.id, "?? ??? ????? ????? ????? ???.", true);
  ...
  switch (main) {
    ...
    case "pdf":
      ...
      break;
    case "admin":
      // The check above already secured this
      await answerCallbackQuery(query.id);
